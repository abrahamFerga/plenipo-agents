...
const PATH_RULES = [
  ...
  // ── Marketplace-local additions ────────────────────────────────────────────
  // RE-APPLY THESE after any re-copy from plugins/plenipo/skills/setup/assets/pr-gates.mjs.
  // They are NOT in the shared asset on purpose: a product's spine is its C# authorization,
  // approval and persistence code, and the four CONTENT_RULES above catch that. This repo ships
  // no code — its spine is the machinery that decides whether a skill is fit to publish. Note
  // what is deliberately absent: a blanket `plugins/**` rule. Authoring skills IS the ordinary
  // work here, and a rule that fires on every pull request is a rule that gets rubber-stamped.
  [/^eng\//, 'the verifier itself (eng/) — an agent that can edit its own checks has none'],
  [/(^|\/)\.claude-plugin\//, 'a plugin or marketplace manifest — a bad one breaks installation everywhere'],
