...
`deliver:product-developer`. `test` delegates its bounded sweep to a Sonnet 5 agent. The
unattended `ship` path uses one revision-bound cloud verdict instead of also launching the local
...
|---|---|---|
| **Where** | an Opus 5 `deliver:product-developer` on your machine (needs Docker) | the revision-bound GitHub Actions verdict; optionally a fresh Sonnet 5 `plenipo:pr-reviewer` for an attended second opinion |
...
- **Cloud review becomes merge authority only in the unattended profile.** At autonomy level 1+,
  the revision-bound `pr-approval-verdict.md` is required; at level 0, the local Sonnet 5
  `plenipo:pr-reviewer` remains an on-demand second opinion with no secret. Do not stack both for
  the same routine review. Install the cloud workflow through
